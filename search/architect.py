...
@LastEditTime: 2019-09-24 14:37:02
...
class Architect(object):
  ...
  def __init__(self, model, optimizer, args):
    self.network_momentum = args.w_momentum
    self.network_weight_decay = args.w_weight_decay
    self.model = model
  ...
  def step(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer, criterion,unrolled):
    self.optimizer.zero_grad()
    # if unrolled:
    #     self._backward_step_unrolled(input_train, target_train, input_valid, target_valid, eta, network_optimizer)
    # else:
    alpha = self._backward_step(input_valid, target_valid, criterion)
    self.optimizer.step()
    new_alpha = self.model.arch_parameters()
    rescaled_alpha = utils.rescale(alpha, new_alpha, self.model.selecteds)
    self.model.update_arch_parameters(rescaled_alpha)

  def _backward_step(self, input_valid, target_valid, criterion):
    loss = criterion(target_valid, self.model(input_valid), self.model.selecteds, self.model.arch_parameters())
    loss.backward()
